...
import { fetchCreateLoos, fetchGetLocation, fetchPersonalProfile } from '../../redux/actions/action';
...
const AddingLooModal = ({ show, handleClose}) => {
    ...
    const idNewLoo = useSelector((state) => {
        return state.createLoo.myLoos.id
    })
            ...
            <Form onSubmit={handleSaveLoo}>
                    ...
                    <ListGroup className='position-absolute' style={{ top: "32%",width: "94%"}}>
                    {
                        showDropdown && resultsResearch && resultsResearch.map((result) => (
                            <ListGroup.Item className='d-flex' key={result.longitude} onClick={() => handleClick(result)}>{result.label}</ListGroup.Item>
                        )) 
                    }
                    </ListGroup> 

                    <p className='p-0 m-0 fs-5 ms-1 text-dark fw-medium'>Longitude:</p>
                    <Form.Control className='mb-4'
                    required
                    ...
                    <p className='p-0 m-0 fs-5 ms-1 text-dark fw-medium'>Latitude:</p>
                    <Form.Control className='mb-4'
                    required
                    placeholder="Autogenerated..."
                    value={form.latitude}
                    ...
                    <Button className="text-primary border border-primary fs-5 fw-medium rounded-pill px-4 shadow-sm "type='submit' variant="secondary">
                    Upload details
                    </Button>
                    {
                        isFormLoaded && (
